...
const Btn = ({ onClick, children, style = {}, color = EVITARE_BLUE }) => (
  <button
    onClick={onClick}
    style={{ padding: "7px 14px", fontSize: 12, fontWeight: 600, border: "none", borderRadius: 8, cursor: "pointer", background: color, color: "#fff", display: "flex", alignItems: "center", gap: 5, ...style }}
  >
    {children}
  </button>
);
...
const Input = ({ label, ...props }) => (
  <div style={{ marginBottom: 12 }}>
    ...
    <input
      {...props}
      style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #E5E7EB", borderRadius: 8, fontSize: 13, outline: "none", boxSizing: "border-box", transition: "border 0.2s", ...props.style }}
...
const Select = ({ label, children, ...props }) => (
  <div style={{ marginBottom: 12 }}>
    {label && <div style={{ fontSize: 11, fontWeight: 600, color: "#374151", marginBottom: 4 }}>{label}</div>}
    <select {...props} style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #E5E7EB", borderRadius: 8, fontSize: 13, outline: "none", background: "#fff", ...props.style }}>
      {children}
    </select>
  </div>
);
...
const EvitareLogo = ({ size = 40 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none">
    <path d="M50 10 C28 10 10 28 10 50 C10 72 28 90 50 90 C72 90 90 72 90 50" stroke={EVITARE_BLUE} strokeWidth="10" strokeLinecap="round" fill="none" />
    <path d="M80 20 L90 50 L60 38" fill={EVITARE_BLUE} />
  </svg>
);
...
function LoginPage({ onLogin }) {
  ...
  async function handleLogin(e) {
    ...
    const { data, error: err } = await supabase
      ...
      .single();
    setLoading(false);
    if (err || !data) { setError("Correo o contrasena incorrectos"); return; }
    onLogin(data);
  }
    ...
    <div style={{ minHeight: "100vh", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter,sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 400, padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 14, color: "#6B7280", margin: 0 }}>Panel de administracion</p>
        </div>
        ...
        <div style={{ background: "#fff", border: "1.5px solid #E5E7EB", borderRadius: 16, padding: "32px 28px", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#111827", marginBottom: 4, textAlign: "center" }}>Iniciar sesion</h2>
          <p style={{ fontSize: 13, color: "#6B7280", textAlign: "center", marginBottom: 24 }}>Ingresa tus credenciales para continuar</p>

          <form onSubmit={handleLogin}>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#374151", marginBottom: 4 }}>Correo electronico</div>
              <input
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#374151", marginBottom: 4 }}>Contrasena</div>
            ...
            <button
              ...
              style={{ width: "100%", padding: "13px", background: loading ? "#93C5FD" : EVITARE_BLUE, color: "#fff", border: "none", borderRadius: 9, fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", transition: "background 0.2s" }}
            >
        ...
        <p style={{ textAlign: "center", fontSize: 12, color: "#9CA3AF", marginTop: 20 }}>
          Evitare Medical Center &copy; {new Date().getFullYear()}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, overflowY: "auto", height: "100%" }}>
      <div style={{ background: "#fff", borderRadius: 14, padding: 20, marginBottom: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: `2px solid ${EVITARE_BLUE}22` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "#6B7280" }}>Panel de administracion - Guadalajara, Jalisco</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 16, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontWeight: 700, color: "#111827", fontSize: 15 }}>Gestion de Usuarios</span>
          ...
          <div key={u.id} style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", display: "flex", alignItems: "center", gap: 16, border: u.id === usuario.id ? `2px solid ${EVITARE_BLUE}` : "2px solid transparent" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                {u.id === usuario.id && <span style={{ fontSize: 10, background: EVITARE_BLUE + "22", color: EVITARE_BLUE, padding: "1px 8px", borderRadius: 10, fontWeight: 700 }}>Tu</span>}
              ...
              {u.rol === "master" && <div style={{ fontSize: 11, color: EVITARE_BLUE, fontWeight: 600 }}>Acceso completo a todos los modulos</div>}
            </div>
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              ...
              {u.id !== usuario.id && (
                <>
                  ...
                  <button onClick={() => deleteUsuario(u.id)} style={{ border: "1.5px solid #FCA5A5", background: "#FEF2F2", color: "#EF4444", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Eliminar</button>
                </>
              )}
            </div>
          </div>
      ...
      {showForm && (
        <Modal title={editItem ? "Editar Usuario" : "Nuevo Usuario"} onClose={() => { setShowForm(false); setEditItem(null); setForm(FORM0); }}>
          <Input label="Nombre completo *" value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} placeholder="Ej. Carlos Ramirez" />
          <Input label="Correo electronico *" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="usuario@evitare.mx" />
          <Input label="Contrasena *" type="text" value={form.password_hash} onChange={e => setForm({ ...form, password_hash: e.target.value })} placeholder="Contrasena de acceso" />
          <Select label="Rol" value={form.rol} onChange={e => setRol(e.target.value)}>
            <option value="operador">Operador (permisos especificos)</option>
            <option value="master">Master (acceso total)</option>
          </Select>

          {form.rol === "operador" && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "#374151", marginBottom: 10 }}>PERMISOS DE MODULOS</div>
...
function SeccionMensajes({ toast }) {
  ...
  const PLATFORMS = {
    ...
    whatsapp:  { label: "WhatsApp",  color: "#25D366", bg: "#E6FAF0" },
  };
  const QUICK_REPLIES = [
    "Hola! Bienvenido a Evitare. En que podemos ayudarte?",
    "Nuestro horario es Lun-Vie 8am-7pm y Sab 9am-3pm.",
    "Con gusto te atiendo. Puedes darme mas detalles?",
    "Gracias por contactarnos. Te respondemos en breve.",
  ];
              ...
              <div key={c.id} onClick={() => selectContact(c.id)} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #F9FAFB", background: selected === c.id ? EVITARE_BLUE + "11" : "transparent", borderLeft: selected === c.id ? `3px solid ${EVITARE_BLUE}` : "3px solid transparent" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    ...
                    <div style={{ position: "absolute", bottom: 0, right: 0, width: 13, height: 13, borderRadius: "50%", background: p.color, border: "2px solid #fff" }} />
                  </div>
                  ...
                  <button onClick={e => { e.stopPropagation(); deleteContact(c.id); }} style={{ border: "none", background: "none", cursor: "pointer", color: "#EF4444", opacity: 0.5 }}>x</button>
                </div>
              </div>
            );
          })}
          ...
          {tab === "chat" ? (
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 20, background: "#F9FAFB" }}>
              <div style={{ background: "#fff", borderRadius: 14, padding: 18, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 14 }}>Informacion</h3>
                {[["Nombre", contact.nombre], ["Telefono", contact.telefono || "-"], ["Plataforma", pl.label], ["Etiqueta", contact.etiqueta], ["Estado", contact.estado]].map(([l, v]) => (
...
function SeccionInventario({ toast }) {
  ...
  const stats = {
    ...
    dosis: vacunas.reduce((a, v) => a + v.dosis_disponibles, 0),
    vencer: vacunas.filter(v => { const d = new Date(v.caducidad); return (d - new Date()) < 30 * 864e5 && d > new Date(); }).length,
    bajos: vacunas.filter(v => v.estado !== "disponible").length
  };
  ...
  return (
    <div style={{ padding: 24, overflowY: "auto", height: "100%" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
        {[
          ["Vacunas", stats.total, "en catalogo", EVITARE_BLUE],
          ["Dosis", stats.dosis, "disponibles", "#10B981"],
          ["Por vencer", stats.vencer, "en 30 dias", "#F59E0B"],
          ["Bajo stock", stats.bajos, "requieren reorden", "#EF4444"]
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          {["catalogo", "movimientos"].map(t => (
            <button key={t} onClick={() => setSubTab(t)} style={{ padding: "6px 14px", fontSize: 12, fontWeight: 600, border: "none", borderRadius: 8, cursor: "pointer", background: subTab === t ? EVITARE_BLUE : "#F3F4F6", color: subTab === t ? "#fff" : "#6B7280" }}>
              {t === "catalogo" ? "Catalogo" : "Movimientos"}
            </button>
          ))}
        ...
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#F9FAFB" }}>
              {["Vacuna", "Lote", "Fabricante", "Caducidad", "Dosis", "Almacen", "Estado", "Acciones"].map(h => (
                  ...
                  <tr key={v.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                    ...
                    <td style={{ padding: "10px 14px" }}>
                      <div style={{ display: "flex", gap: 4 }}>
                        <button onClick={() => { setEditItem(v); setForm({ ...v, caducidad: v.caducidad || "" }); setShowForm(true); }} style={{ border: "none", background: EVITARE_BLUE + "22", color: EVITARE_BLUE, borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Editar</button>
                        <button onClick={() => deleteVacuna(v.id)} style={{ border: "none", background: "#FEF2F2", color: "#EF4444", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Eliminar</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {subTab === "movimientos" && (
        <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#F9FAFB" }}>
              {["Fecha", "Tipo", "Vacuna", "Dosis", "Almacen", "Motivo", "Personal"].map(h => (
                <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#6B7280", borderBottom: "1px solid #E5E7EB" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
      ...
      {showForm && (
        <Modal title={editItem ? "Editar Vacuna" : "Nueva Vacuna"} onClose={() => { setShowForm(false); setEditItem(null); setForm(VFORM); }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <Input label="Stock minimo" type="number" value={form.stock_minimo} onChange={e => setForm({ ...form, stock_minimo: e.target.value })} />
          </div>
          <Select label="Almacen" value={form.almacen} onChange={e => setForm({ ...form, almacen: e.target.value })}>
            {ALMACENES.map(a => <option key={a} value={a}>{a}</option>)}
          </Select>
      ...
      {showMov && (
        <Modal title={`Registrar ${showMov.charAt(0).toUpperCase() + showMov.slice(1)}`} onClose={() => setShowMov(null)}>
          ...
          <Select label="Almacen origen" value={mform.almacen_origen} onChange={e => setMform({ ...mform, almacen_origen: e.target.value })}>
            {ALMACENES.map(a => <option key={a} value={a}>{a}</option>)}
          </Select>
          {showMov === "transferencia" && (
            <Select label="Almacen destino" value={mform.almacen_destino} onChange={e => setMform({ ...mform, almacen_destino: e.target.value })}>
              <option value="">Seleccionar...</option>
              {ALMACENES.map(a => <option key={a} value={a}>{a}</option>)}
            </Select>
          )}
...
function SeccionCalendario({ toast }) {
  ...
  useEffect(() => {
    ...
    s2.onload = () => {
      tokenClientRef.current = window.google.accounts.oauth2.initTokenClient({
        ...
        scope: GOOGLE_SCOPES,
        callback: async (resp) => { if (resp.error) return; setSignedIn(true); await loadEvents(); }
      });
      setGisLoaded(true);
    };
  ...
  async function saveEvent() {
    ...
    const event = {
      summary: form.titulo, description: form.descripcion, location: form.lugar,
      start: { dateTime: `${form.fecha}T${form.horaInicio}:00`, timeZone: "America/Mexico_City" },
      end:   { dateTime: `${form.fecha}T${form.horaFin}:00`,    timeZone: "America/Mexico_City" }
    };
    try {
  ...
  const diasS     = ["Dom", "Lun", "Mar", "Mie", "Jue", "Vie", "Sab"];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#fff" }}>
      <div style={{ padding: "12px 20px", borderBottom: "1px solid #E5E7EB", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        <button onClick={loadEvents} style={{ border: "1px solid #E5E7EB", background: "#fff", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12 }}>Actualizar</button>
      </div>
      <div style={{ flex: 1, overflowY: "auto" }}>
                ...
                {dayEvs.map((ev, ei) => (
                  <div key={ev.id}
                    onClick={() => {
                      ...
                      const e2 = ev.end.dateTime ? new Date(ev.end.dateTime) : null;
                      setEditEvent(ev);
                      setForm({ titulo: ev.summary || "", fecha: (ev.start.dateTime || ev.start.date)?.slice(0, 10) || "", horaInicio: s ? `${String(s.getHours()).padStart(2, "0")}:${String(s.getMinutes()).padStart(2, "0")}` : "09:00", horaFin: e2 ? `${String(e2.getHours()).padStart(2, "0")}:${String(e2.getMinutes()).padStart(2, "0")}` : "10:00", descripcion: ev.description || "", lugar: ev.location || "" });
                      setShowForm(true);
                    }}
                    style={{ position: "absolute", left: 2, right: 2, top: getTop(ev) + "px", height: Math.max(getHeight(ev), 22) + "px", background: COLORS[ei % COLORS.length] + "22", borderLeft: `3px solid ${COLORS[ei % COLORS.length]}`, borderRadius: "0 6px 6px 0", padding: "2px 6px", cursor: "pointer", overflow: "hidden", zIndex: 1 }}
                  >
      ...
      {events.filter(ev => (ev.start.dateTime || ev.start.date)?.slice(0, 10) === viewDate.toISOString().slice(0, 10)).length > 0 && (
        <div style={{ borderTop: "1px solid #E5E7EB", padding: "10px 16px", background: "#F9FAFB", flexShrink: 0, maxHeight: 180, overflowY: "auto" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#6B7280", marginBottom: 8 }}>
            CITAS - {viewDate.toLocaleDateString("es", { weekday: "long", day: "numeric", month: "long" })}
          </div>
          {events.filter(ev => (ev.start.dateTime || ev.start.date)?.slice(0, 10) === viewDate.toISOString().slice(0, 10)).map((ev, i) => (
            <div key={ev.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "#fff", borderRadius: 10, marginBottom: 6, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
              ...
              <button onClick={() => deleteEvent(ev.id)} style={{ border: "none", background: "#FEF2F2", color: "#EF4444", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11 }}>Eliminar</button>
            </div>
          ))}
        </div>
      )}
      ...
      {showForm && (
        <Modal title={editEvent ? "Editar Cita" : "Nueva Cita"} onClose={() => { setShowForm(false); setEditEvent(null); setForm(EFORM); }}>
          <Input label="Titulo / Paciente *" value={form.titulo} onChange={e => setForm({ ...form, titulo: e.target.value })} placeholder="Ej. Vacunacion - Maria Gonzalez" />
          ...
          {editEvent && <button onClick={() => deleteEvent(editEvent.id)} style={{ width: "100%", marginTop: 8, padding: "8px", background: "#FEF2F2", color: "#EF4444", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Eliminar esta cita</button>}
        </Modal>
      )}
    </div>
  );
...
function SeccionLeads({ toast }) {
  ...
  async function saveLead() {
    ...
    if (editItem) {
      ...
      const { data } = await supabase.from("contactos").insert({ ...form, plataforma: "whatsapp", ultimo_mensaje: "Lead manual", tiempo: "Ahora", mensajes_sin_leer: 0 }).select().single();
  ...
  return (
    <div style={{ padding: 24, overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead><tr style={{ background: "#F9FAFB" }}>
            {["Nombre", "Telefono", "Servicio", "Fuente", "Estado", "Fecha", "Acciones"].map(h => (
              <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "#6B7280", borderBottom: "1px solid #E5E7EB" }}>{h}</th>
            ))}
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "#9CA3AF" }}>Cargando...</td></tr>
              : leads.map(l => (
                <tr key={l.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => { setEditItem(l); setForm({ nombre: l.nombre, servicio: l.servicio || "", zona: l.zona || "", fuente: l.fuente || "WhatsApp", estado: l.estado, telefono: l.telefono || "", notas: l.notas || "" }); setShowForm(true); }} style={{ border: "none", background: EVITARE_BLUE + "22", color: EVITARE_BLUE, borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Editar</button>
                      <button onClick={() => deleteLead(l.id)} style={{ border: "none", background: "#FEF2F2", color: "#EF4444", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showForm && (
        <Modal title={editItem ? "Editar Lead" : "Nuevo Lead"} onClose={() => { setShowForm(false); setEditItem(null); setForm(FORM0); }}>
          ...
          <Select label="Servicio" value={form.servicio} onChange={e => setForm({ ...form, servicio: e.target.value })}>
            <option value="">Seleccionar...</option>
            {["Vacunacion", "Tamiz Neonatal", "Ruta a Domicilio", "Nutricion", "Analisis Clinicos", "Paquete Familiar", "Paquete Bebe", "Paquete Senior"].map(s => <option key={s} value={s}>{s}</option>)}
          </Select>
          ...
          <Select label="Fuente" value={form.fuente} onChange={e => setForm({ ...form, fuente: e.target.value })}>
            {["WhatsApp", "Facebook", "Instagram", "Referido", "Llamada", "Pagina web", "Otro"].map(s => <option key={s} value={s}>{s}</option>)}
          </Select>
...
function SeccionFacturacion({ toast }) {
  ...
  async function nextFolio() {
    ...
    if (data && data.length) { const n = parseInt(data[0].folio.replace("FAC-", "")) + 1; return "FAC-" + String(n).padStart(4, "0"); }
    return "FAC-0001";
  }
  ...
  return (
    <div style={{ padding: 24, overflowY: "auto", height: "100%" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14, marginBottom: 22 }}>
        {[
          ["Emitidas", facturas.filter(f => f.estado === "Timbrada").length, "este mes", "#10B981"],
          ["Total", "$" + total.toLocaleString(), "timbradas", EVITARE_BLUE],
          ["Pendientes", facturas.filter(f => f.estado === "Pendiente").length, "por timbrar", "#F59E0B"]
        ].map(([label, val, sub, col]) => (
          <div key={label} style={{ background: "#fff", borderRadius: 12, padding: "16px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", borderTop: `3px solid ${col}` }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: col }}>{val}</div>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#374151" }}>{label}</div>
      ...
      <div style={{ background: "#E0F2FE", borderRadius: 12, padding: "14px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "#0369A1" }}>Facturacion electronica - Guadalajara</div>
        </div>
            ...
            {loading ? <tr><td colSpan={7} style={{ padding: 24, textAlign: "center", color: "#9CA3AF" }}>Cargando...</td></tr>
              : facturas.map(f => (
                <tr key={f.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                  ...
                  <td style={{ padding: "10px 14px" }}>
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => { setEditItem(f); setForm({ folio: f.folio, paciente: f.paciente, servicio: f.servicio || "", monto: f.monto, fecha: f.fecha, estado: f.estado }); setShowForm(true); }} style={{ border: "none", background: EVITARE_BLUE + "22", color: EVITARE_BLUE, borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Editar</button>
                      <button onClick={() => deleteFactura(f.id)} style={{ border: "none", background: "#FEF2F2", color: "#EF4444", borderRadius: 6, padding: "4px 8px", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
...
export default function App() {
  ...
  const NAV = [
    { id: "mensajes",      label: "Mensajes",      show: permisos.mensajes },
    { id: "inventario",    label: "Inventario",    show: permisos.inventario },
    { id: "calendario",    label: "Citas",         show: permisos.calendario },
    { id: "leads",         label: "Leads",         show: permisos.leads },
    { id: "facturacion",   label: "Facturacion",   show: permisos.facturacion },
    { id: "configuracion", label: "Configuracion", show: usuario.rol === "master" },
  ].filter(n => n.show);
      ...
      <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        {!NAV.find(n => n.id === section) && (
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#9CA3AF" }}>
            <EvitareLogo size={48} />
            <p style={{ fontSize: 14 }}>No tienes permiso para ver este modulo.</p>
          </div>
        )}
      </div>
